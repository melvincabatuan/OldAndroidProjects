...
import com.example.thesis.data.ArticleCategory;
import com.example.thesis.data.Author;
import com.example.thesis.data.Category;
...
public class MainActivity extends Activity {
		...
		View.OnClickListener listener = new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				MainActivity.this.startActivity(new Intent(MainActivity.this,
						...
						ArticleActivity.ARTICLE_ID, article.getId()));
			}
		};
	...
	private static class LoadArticleListTask extends
		...
		protected ArticleListModel doInBackground(Void... voids) {
			...
			return helper.callInTransaction(new Callable<ArticleListModel>() {
				...
				public ArticleListModel call() throws SQLException {
					...
					/*
					 * This activity will display all categories and articles.
					 * So, we will query the cross reference table, and use the
					 * ORM to fill in our data objects.
					 */
					...
					for (ArticleCategory mapping : articleCategoryDao
						...
						if (allCategories.containsKey(key))
							allCategories.get(key).addArticle(
									mapping.getArticle());
						else {
		...
		/**
		 * Called for the first run of the application (or when all Articles
		 * have been deleted).
		 * 
		 * @param helper
		...
		private void insertSampleData(DatabaseHelper helper)
			...
			for (int i = 0; i < SAMPLE_COUNT; i++) {
				...
				if (i == 0) {
					...
					for (Author author : authors)
						articleAuthorDao.create(new ArticleAuthor(author,
								article));
				} else {
